...
/**
 * Core release-reservation logic, extracted from the onTaskDispatched handler so
 * it can be invoked directly from createReservation's emulator-only setTimeout
 * fallback when Cloud Tasks enqueue throws (the Firebase emulator suite ships no
 * Cloud Tasks emulator). Production still drives this via the onTaskDispatched
 * wrapper below.
 *
 * Idempotent: short-circuits when reservation is missing, status !== "active",
 * or now < expiresAt — late-firing emulator timers are safe no-ops.
 */
export async function releaseReservationCore(
  args: ReleaseReservationCoreArgs
): Promise<void> {
  const { reservationId, db } = args;
