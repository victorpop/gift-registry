/**
 * Tests for getReservationForItem callable (quick-260513-g9g Task 1).
 * Covers: signed-in and guest query paths, defence-in-depth, empty results, error cases.
 *
 * Mock harness mirrors hydrateActiveReservation.test.ts extended with itemId support.
 */
...
// Import AFTER mocks
import { getReservationForItem } from "../reservation/getReservationForItem";
...
describe("getReservationForItem callable", () => {
  it("Test G-01 (happy — signed-in, item match): returns active reservation scoped to itemId", async () => {
  ...
  it("Test G-02 (happy — guest, item match): returns active reservation for guest by email", async () => {
    const result = await getReservationForItem.run(
      makeRequest({ registryId: "reg1", itemId: "it2", giverEmail: "guest@example.com" }, null)
    );
  ...
  it("Test G-03 (signed-in defence-in-depth): uid wins, giverEmail ignored, returns res1", async () => {
    // Signed-in caller passes giverEmail — should still return res1 (uid match), not resGuest
    const result = await getReservationForItem.run(
      makeRequest({ registryId: "reg1", itemId: "it1", giverEmail: "guest@example.com" }, { uid: "u1" })
    );
  ...
  it("Test G-04 (wrong itemId for signed-in user): returns {active: null} — it2 belongs to different reservation", async () => {
    // u1 has a reservation on it1, NOT it2
    const result = await getReservationForItem.run(
      makeRequest({ registryId: "reg1", itemId: "it2" }, { uid: "u1" })
    );
  ...
  it("Test G-06 (missing registryId): throws invalid-argument MISSING_REGISTRY_ID", async () => {
    await expect(
      getReservationForItem.run(makeRequest({ itemId: "it1" }, { uid: "u1" }))
  ...
  it("Test G-07 (missing itemId): throws invalid-argument MISSING_ITEM_ID", async () => {
    await expect(
      getReservationForItem.run(makeRequest({ registryId: "reg1" }, { uid: "u1" }))
    ).rejects.toMatchObject({
      code: "invalid-argument",
  ...
  it("Test G-08 (guest missing giverEmail): throws invalid-argument MISSING_GIVER_EMAIL", async () => {
    await expect(
      getReservationForItem.run(makeRequest({ registryId: "reg1", itemId: "it2" }, null))
    ).rejects.toMatchObject({
      code: "invalid-argument",
  ...
  it("Test G-09 (no active reservation for user/item): returns {active: null}", async () => {
    const result = await getReservationForItem.run(
      makeRequest({ registryId: "reg1", itemId: "it1" }, { uid: "u_no_reservation" })
    );
